...
from ShadkingKit_python2.UIs import mainUI, shader_list, file_formats
...
class MainWindow(MayaQWidgetDockableMixin, QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.texture_pass_list = {'Base Color': (
        'base_color', ['basecolor', 'diffuse', 'albedo', 'base_color', 'base color'], '.outColor', '.baseColor'),
                                  'Specular R.': (
                                  'spec_roughness', ['spec', 'roughness'], '.outColorR', '.specularRoughness'),
                                  'Metalness': (
                                  'metalness', ['metal', 'metalness', 'metallic'], '.outColorR', '.metalness'),
                                  ...
                                  'Transmis.': ('transmission',
                                                ['transmission', 'transmissionweight', 'transmission_weight',
                                                 'transmission weight'], '.outColorR', '.transmission'),
        ...
        self.save_scene_callBack = OpenMaya.MSceneMessage.addCallback(OpenMaya.MSceneMessage.kAfterSave,
                                                                      self.reset_save)
        self.current_time = 0
        ...
        self.ui.assign_to_selected_default.clicked.connect(
            partial(self.change_but_color, self.ui.assign_to_selected_default))
        ...
        self.ui.selectDirectory.setStyleSheet(u"QPushButton{"
                                              ...
                                              "background-image: url(" + script_folder.replace("\\",
                                                                                               "/") + "/icons/folder.png);"
    ...
    def checkOnOff(self):
        ...
        if self.ui.expand_Utils.isChecked():
            ...
            self.bot_layout_anim.setStartValue(
                QtCore.QRect(bot_layout.x(), 145, bot_layout.width(), bot_layout.height()))
            ...
            self.bot_layout_anim.setEasingCurve(QtCore.QEasingCurve.InCurve)
            self.bot_layout_anim.setStartValue(
                QtCore.QRect(bot_layout.x(), 280, bot_layout.width(), bot_layout.height()))
    ...
    def create_delete_aovs(self, value, *args):
        ...
        for aov_but in self.ui.AOV_buts.buttons():
            if aov_but.isChecked():
                selection.append((self.texture_pass_list[aov_but.text()][0], self.texture_pass_list[aov_but.text()][1],
                                  self.texture_pass_list[aov_but.text()][3]))
        if selection:
            ...
            if value:
                    ...
                    if aov_name[0] not in existed_aovs_names:
                        ...
                        for pair in shading_groups:
                            ...
                            for customAOV in range(0, numOfCustomAOVs):
                                if cmds.getAttr(pair[1] + ".aiCustomAOVs[{}].aovName".format(customAOV)) == aov_name[0]:
                ...
                if already_exist:
                    cmds.confirmDialog(t='Warning', m="AOVs {} already exist".format(already_exist), icn='information')
            else:
                ...
                if len(custom_aovs) > 0:
                    ...
                    if len(not_found_aovs) != 0:
                        cmds.confirmDialog(t='Warning', m="AOVs {} not found".format(not_found_aovs),
                                           icn='information')
                else:
